function DrawStartup(app,CallerApp)
    ...
    try
        ...
        app.CheckBoxesSA.ValueChangedFcn = @(src,event)Cauer3D.Plot.CheckBoxesSAValueChanged(src,event);
        for Row = 1:CheckBoxRowNum
            app.CheckBoxesSL{Row}.UserData.app = app;
            app.CheckBoxesSL{Row}.ValueChangedFcn = @(src,event)Cauer3D.Plot.CheckBoxesSLValueChanged(src,event);
        end
        for Col = 1:CheckBoxColNum
            app.CheckBoxesSP{Col}.UserData.app = app;
            app.CheckBoxesSP{Col}.ValueChangedFcn = @(src,event)Cauer3D.Plot.CheckBoxesSPValueChanged(src,event);
        end
    catch ME
    ...
    app.Circuit.ButtonPushedFcn = @(~,~) Cauer3D.Plot.CircuitDraw(app);
    app.UIFigure.CloseRequestFcn = @(~,~) Cauer3D.Plot.DrawCloseApp(app);
    close(f);
end
